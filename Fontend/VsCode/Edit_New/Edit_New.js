document.addEventListener('DOMContentLoaded', function() {
    ...
    function addMovie(title, image, video, description) {
        const movieId = Date.now();
        movies[movieId] = { title, image, video, description };
        renderGallery();
    }
    ...
    function renderGallery() {
        ...
        for (const id in movies) {
            ...
            item.innerHTML = `<img src="${movie.image}" alt="${movie.title}" onclick="showDetail(${id})" />`;
    ...
    window.showDetail = function(id) {
        ...
        movieDescriptionText.innerText = movie.description;
        currentMovieId = id;
        hasEdited = false; 
    ...
    window.confirmDelete = function() {
        if (confirm("คุณต้องการลบหนังเรื่องนี้ใช่ไหม?")) {
            delete movies[currentMovieId];
            movieDetail.style.display = 'none';
            currentMovieId = null; // รีเซ็ต currentMovieId หลังจากลบ
            renderGallery();
        }
    };
    ...
    window.openModal = function() {
        if (currentMovieId) {
            ...
            document.getElementById('movie-image-input').value = movie.image;
            document.getElementById('movie-video-input').value = movie.video;
            document.getElementById('movie-description-input').value = movie.description;
            ...
            document.getElementById('movie-video-input').value = '';
            document.getElementById('movie-description-input').value = '';
        }
    ...
    window.saveChanges = function() {
        const titleInput = document.getElementById('movie-title-input').value;
        const imageInput = document.getElementById('movie-image-input').value;
        const videoInput = document.getElementById('movie-video-input').value;
        const descriptionInput = document.getElementById('movie-description-input').value;

        if (titleInput && imageInput && videoInput && descriptionInput) {
            if (currentMovieId) {
                movies[currentMovieId] = { title: titleInput, image: imageInput, video: videoInput, description: descriptionInput };
                hasEdited = true; 
                backButton.innerText = 'Done'; 
            } else {
                addMovie(titleInput, imageInput, videoInput, descriptionInput);
            }
            closeModal();
            renderGallery();
        } else {
